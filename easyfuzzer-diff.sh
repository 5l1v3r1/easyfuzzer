...
echo "<tr><td><a href=\"$MAIN\">$MAIN</a></td><td><a href=\"${MAIN}-out\">${MAIN}-out</a></td><td>`wc -l $MAIN|awk '{print$1}'`</td><td><a href=\"$F.0-req\">$F.0-req</a></td><td><a href=\"$F.0-resp\">$F.0-resp</a></td><td>`wc -c $F.0-clean_resp|awk '{print$1}'`</td><td>---</td><td>---</td></tr>" >> $F.html
for i in `wc -l $F.*.md5.txt|grep .md5.txt|sort -n -r|awk '{print$2}'`; do
  ...
  test "$k" = "$F.0-resp" || {
    ...
    echo "<tr><td><a href=\"$i\">$i</a></td><td><a href=\"${i}-out\">${i}-out</a></td><td>`wc -l $i|awk '{print$1}'`</td><td><a href=\"`echo $k|sed 's/resp/req/'`\">`echo $k|sed 's/resp/req/'`</a></td><td><a href=\"$k\">$k</a></td><td>`wc -c $j|awk '{print$1}'`</td><td><a href=\"$i.diff.txt\">$i.diff.txt</a></td><td>`wc -c $i.diff.txt|awk '{print$1}'`</td></tr>" >> $F.html
    {
      echo ====================================================================
